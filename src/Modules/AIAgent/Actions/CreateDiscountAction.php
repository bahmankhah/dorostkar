...
class CreateDiscountAction extends AbstractAction
{
    ...
    public function execute(array $data)
    {
        ...
        try {
            ...
            return $this->success(__('کد تخفیف با موفقیت ایجاد شد', 'dorostkar'), [
                'coupon_id' => $couponId,
                'code' => $code,
                'amount' => $amount,
                'type' => $type,
            ...
            return $this->error(__('خطا در ایجاد کد تخفیف: ', 'dorostkar') . $e->getMessage());
        }
    }
    ...
    public function validate(array $data)
    {
        ...
        if (floatval($amount) <= 0) {
            $errors[] = __('مقدار تخفیف باید بیشتر از صفر باشد', 'dorostkar');
        }
        ...
        if (!in_array($type, $validTypes)) {
            $errors[] = __('نوع تخفیف نامعتبر است', 'dorostkar');
        }

        // Validate percent amount
        if ($type === 'percent' && floatval($amount) > 100) {
            $errors[] = __('درصد تخفیف نمی‌تواند بیشتر از ۱۰۰ باشد', 'dorostkar');
        }
