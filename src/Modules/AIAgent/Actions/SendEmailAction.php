...
class SendEmailAction extends AbstractAction
{
    ...
    public function execute(array $data)
    {
        ...
        if (empty($email) || !is_email($email)) {
            return $this->error(__('آدرس ایمیل معتبر یافت نشد', 'dorostkar'));
        }
        
        if (empty($subject)) {
            return $this->error(__('موضوع ایمیل الزامی است', 'dorostkar'));
        }
        
        if (empty($message)) {
            return $this->error(__('متن ایمیل الزامی است', 'dorostkar'));
        }
        ...
        if ($sent) {
            return $this->success(__('ایمیل با موفقیت ارسال شد', 'dorostkar'), [
                'email' => $email,
                'subject' => $subject,
            ]);
        }

        return $this->error(__('خطا در ارسال ایمیل', 'dorostkar'));
    }
    ...
    public function validate(array $data)
    {
        ...
        if (empty($this->getField($data, 'subject', ''))) {
            $errors[] = __('موضوع ایمیل الزامی است', 'dorostkar');
        }
        
        // Check for message
        if (empty($this->getField($data, 'message', ''))) {
            $errors[] = __('متن ایمیل الزامی است', 'dorostkar');
        }
        ...
        if (empty($email) && empty($customerId) && !($entityType === 'customer' && $entityId)) {
            $errors[] = __('آدرس ایمیل یا شناسه مشتری الزامی است', 'dorostkar');
        }
