...
class SendSmsAction extends AbstractAction
{
    ...
    public function execute(array $data)
    {
        ...
        if (empty($message)) {
            return $this->error(__('متن پیامک الزامی است', 'dorostkar'));
        }
        ...
        if (empty($phone)) {
            return $this->error(__('شماره تلفن یافت نشد', 'dorostkar'));
        }
        ...
        if (empty($smsProvider)) {
            ...
            return $this->success(__('پیامک برای ارسال دستی ذخیره شد (ارائه‌دهنده پیامک پیکربندی نشده)', 'dorostkar'), [
                'id' => $smsId,
                'phone' => $phone,
                'message' => $message,
                'manual' => true,
        ...
        if ($sent) {
            return $this->success(__('پیامک با موفقیت ارسال شد', 'dorostkar'), [
                'phone' => $phone,
                'message' => $message,
            ]);
        }
        ...
        return $this->success(__('پیامک برای ارسال دستی ذخیره شد', 'dorostkar'), [
            'id' => $smsId,
            'phone' => $phone,
            'message' => $message,
            'manual' => true,
    ...
    public function validate(array $data)
    {
        ...
        if (empty($this->getField($data, 'message', ''))) {
            $errors[] = __('متن پیامک الزامی است', 'dorostkar');
        }
        ...
        if (empty($phone) && empty($customerId) && !($entityType === 'customer' && $entityId)) {
            $errors[] = __('شماره تلفن یا شناسه مشتری الزامی است', 'dorostkar');
        }
