 ...
 * 
 * @package Dorostkar\Modules\AIAgent\Actions
 */

namespace Dorostkar\Modules\AIAgent\Actions;
...
class UpdateProductAction extends AbstractAction
{
    ...
    public function execute(array $data)
    {
        ...
        if (!$productId) {
            return $this->error(__('شناسه محصول یافت نشد', 'dorostkar'));
        }
        ...
        if (!$product) {
            return $this->error(__('محصول یافت نشد', 'dorostkar'));
        }
        ...
        if (empty($updated)) {
            return $this->error(__('هیچ فیلدی برای بروزرسانی مشخص نشده', 'dorostkar'));
        }

        try {
            $product->save();

            return $this->success(__('محصول با موفقیت بروزرسانی شد', 'dorostkar'), [
                'product_id' => $productId,
                'updated_fields' => $updated,
            ]);
        } catch (\Exception $e) {
            return $this->error(__('خطا در بروزرسانی محصول: ', 'dorostkar') . $e->getMessage());
        }
    }
    ...
    public function validate(array $data)
    {
        ...
        if (!$productId) {
            $errors[] = __('شناسه محصول الزامی است', 'dorostkar');
        } else {
            // Validate product exists
            $product = wc_get_product(absint($productId));
            if (!$product) {
                $errors[] = __('محصول یافت نشد', 'dorostkar');
            }
        }
        ...
        if ($newPrice !== null && floatval($newPrice) < 0) {
            $errors[] = __('قیمت نمی‌تواند منفی باشد', 'dorostkar');
        }

        $salePrice = $this->getField($data, 'sale_price');
        if ($salePrice !== null && $salePrice !== '' && floatval($salePrice) < 0) {
            $errors[] = __('قیمت حراج نمی‌تواند منفی باشد', 'dorostkar');
        }
