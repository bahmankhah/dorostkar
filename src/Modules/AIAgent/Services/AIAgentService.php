 ...
 * 
 * @package Dorostkar\Modules\AIAgent\Services
 */

namespace Dorostkar\Modules\AIAgent\Services;

use Dorostkar\Modules\AIAgent\Services\LLM\LLMFactory;
use Dorostkar\Modules\AIAgent\Exceptions\SubscriptionLimitException;
use Dorostkar\Modules\AIAgent\Exceptions\FeatureDisabledException;
use Dorostkar\Modules\AIAgent\Exceptions\LLMConnectionException;
...
class AIAgentService
{
        ...
        try {
            ...
            if ($type === 'all' || $type === 'products') {
                if ($this->subscription->isFeatureEnabled(SubscriptionManager::FEATURE_PRODUCT_ANALYSIS)) {
                    ...
                    $results['warnings'][] = __('قابلیت تحلیل محصولات در اشتراک شما فعال نیست', 'dorostkar');
                }
            }
            ...
            if ($type === 'all' || $type === 'customers') {
                if ($this->subscription->isFeatureEnabled(SubscriptionManager::FEATURE_CUSTOMER_ANALYSIS)) {
                    ...
                    $results['warnings'][] = __('قابلیت تحلیل مشتریان در اشتراک شما فعال نیست', 'dorostkar');
                }
            }
    ...
    private function validateAnalysisPrerequisites($type)
    {
        ...
        if (!$this->subscription->isModuleEnabled()) {
            throw new FeatureDisabledException(__('ماژول دستیار هوشمند فعال نیست', 'dorostkar'));
        }

        // Check usage limits
        if ($this->subscription->isLimitExceeded('analyses_per_day')) {
            throw new SubscriptionLimitException(__('محدودیت تحلیل روزانه به پایان رسیده است', 'dorostkar'));
        }
        ...
        if (!$validation['valid']) {
            throw new LLMConnectionException(__('پیکربندی LLM نامعتبر است: ', 'dorostkar') . implode(', ', $validation['errors']));
        }
    }
    ...
    public function analyzeSingle($type, $entityId)
    {
        ...
        if (!$this->subscription->isModuleEnabled()) {
            return ['success' => false, 'error' => __('ماژول فعال نیست', 'dorostkar')];
        }
    ...
    private function executeApprovedActions($limit = null)
    {
        ...
        foreach ($actions as $action) {
            ...
            if ($usageLimit['remaining'] <= 0 && $usageLimit['allowed'] !== -1) {
                $errors[] = __('محدودیت اقدامات روزانه به پایان رسیده است', 'dorostkar');
                break;
            }
            ...
            try {
                ...
                if ($result['success']) {
                    ...
                    $errors[] = sprintf(__('اقدام %d: ', 'dorostkar'), $action['id']) . ($result['message'] ?: __('خطای ناشناخته', 'dorostkar'));
                }
            } catch (\Exception $e) {
                $errors[] = sprintf(__('اقدام %d: ', 'dorostkar'), $action['id']) . $e->getMessage();
            }
        }
    ...
    public function executeActions($limit = null)
    {
        ...
        if (!$this->subscription->isFeatureEnabled(SubscriptionManager::FEATURE_AUTO_ACTIONS)) {
            return [
                'success' => false,
                'error' => __('قابلیت اجرای خودکار اقدامات در اشتراک شما فعال نیست', 'dorostkar'),
                'executed' => 0,
            ];
        }
        ...
        if ($usageLimit['remaining'] <= 0 && $usageLimit['allowed'] !== -1) {
            return [
                'success' => false,
                'error' => __('محدودیت اقدامات روزانه به پایان رسیده است', 'dorostkar'),
                'executed' => 0,
            ];
        }
        ...
        foreach ($actions as $action) {
            try {
                ...
                if ($result['success']) {
                    ...
                    $failed++;
                    $errors[] = $result['message'] ?: $result['error'] ?: __('خطای ناشناخته', 'dorostkar');
                }
            } catch (\Exception $e) {
                $failed++;
                $errors[] = $e->getMessage();
