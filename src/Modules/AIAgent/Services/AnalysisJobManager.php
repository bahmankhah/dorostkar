...
class AnalysisJobManager
{
    ...
    public function startJob($type = 'all')
    {
        ...
        if ($currentJob['status'] === self::STATUS_RUNNING) {
            ...
            if (($now - $lastUpdate) > self::STALE_JOB_THRESHOLD) {
                ...
                return [
                    'success' => false,
                    'error' => __('یک تحلیل در حال اجرا است. لطفاً صبر کنید یا آن را لغو کنید.', 'dorostkar'),
                ];
            }
        }
        ...
        if (!$this->subscription->isModuleEnabled()) {
            return [
                'success' => false,
                'error' => __('ماژول دستیار هوشمند فعال نیست', 'dorostkar'),
            ];
        }
        ...
        if ($totalItems === 0) {
            ...
            if (!$includeProducts && !$includeCustomers) {
                $reasons[] = __('تحلیل محصولات و مشتریان در تنظیمات غیرفعال است', 'dorostkar');
            } else {
                if ($includeProducts && empty($products)) {
                    $reasons[] = __('هیچ محصول منتشر شده‌ای یافت نشد', 'dorostkar');
                }
                if ($includeCustomers && empty($customers)) {
                    $reasons[] = __('هیچ مشتری‌ای یافت نشد', 'dorostkar');
                }
            }
            
            $errorMsg = !empty($reasons) 
                ? implode('. ', $reasons) 
                : __('هیچ موردی برای تحلیل یافت نشد', 'dorostkar');
        ...
        return [
            ...
            'job_id' => $jobId,
            'message' => sprintf(
                __('تحلیل شروع شد. %d مورد در صف تحلیل قرار گرفت.', 'dorostkar'),
                $totalItems
            ),
        ];
    }
            ...
            try {
                ...
                if ($result['success']) {
                    ...
                    $jobState['errors'][] = [
                        'type' => 'product',
                        'id' => $productId,
                        'error' => $result['error'] ?? __('خطای ناشناخته', 'dorostkar'),
                    ];
    ...
    public function cancelJob()
    {
        ...
        if ($jobState['status'] !== self::STATUS_RUNNING) {
            return [
                'success' => false,
                'error' => __('هیچ تحلیلی در حال اجرا نیست', 'dorostkar'),
            ];
        }
        ...
        return [
            'success' => true,
            'message' => __('درخواست لغو تحلیل ارسال شد', 'dorostkar'),
        ];
    }
    ...
    public function resumeStaleJob()
    {
        ...
        if ($jobState['status'] !== self::STATUS_RUNNING) {
            return [
                'success' => false,
                'error' => __('هیچ کار در حال اجرایی برای بازیابی وجود ندارد', 'dorostkar'),
            ];
        }
        ...
        if (($now - $lastUpdate) <= self::STALE_JOB_THRESHOLD) {
            return [
                'success' => false,
                'error' => __('کار هنوز فعال است و نیازی به بازیابی ندارد', 'dorostkar'),
            ];
        }
        ...
        return [
            'success' => true,
            'message' => __('کار با موفقیت بازیابی شد', 'dorostkar'),
        ];
    }
