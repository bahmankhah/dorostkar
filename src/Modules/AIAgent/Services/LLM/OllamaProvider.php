 ...
 * 
 * @package Dorostkar\Modules\AIAgent\Services\LLM
 */

namespace Dorostkar\Modules\AIAgent\Services\LLM;

use Dorostkar\Modules\AIAgent\Contracts\LLMProviderInterface;
...
class OllamaProvider implements LLMProviderInterface
{
    ...
    private function parseResponse($response, $duration)
    {
        if (is_wp_error($response)) {
            return [
                'success' => false,
                'data' => null,
                'error' => __('خطای اتصال: ', 'dorostkar') . $response->get_error_message(),
            ];
        }
        ...
        if (empty($responseBody)) {
            return [
                'success' => false,
                'data' => null,
                'error' => sprintf(__('پاسخ خالی از سرور (کد وضعیت: %d)', 'dorostkar'), $statusCode),
            ];
        }
        ...
        if ($statusCode !== 200) {
            ...
            return [
                'success' => false,
                'data' => null,
                'error' => sprintf(__('خطای HTTP %d: %s', 'dorostkar'), $statusCode, $errorMsg),
            ];
        }
        ...
        if ($data === null) {
            return [
                'success' => false,
                'data' => null,
                'error' => __('پاسخ JSON نامعتبر از سرور', 'dorostkar'),
            ];
        }
    ...
    public function testConnection()
    {
        ...
        if (is_wp_error($response)) {
            return [
                'success' => false,
                'message' => __('اتصال ناموفق: ', 'dorostkar') . $response->get_error_message(),
            ];
        }
        ...
        if ($statusCode !== 200) {
            return [
                'success' => false,
                'message' => sprintf(__('اتصال ناموفق با کد وضعیت %d', 'dorostkar'), $statusCode),
            ];
        }
        ...
        if (!empty($models) && !in_array($configuredModel, $models)) {
            return [
                'success' => true,
                'message' => sprintf(
                    __('اتصال به Ollama برقرار شد، اما مدل "%s" نصب نیست. مدل‌های موجود: %s', 'dorostkar'),
                    $configuredModel,
        ...
        return [
            'success' => true,
            'message' => __('اتصال به Ollama با موفقیت برقرار شد', 'dorostkar'),
        ];
    }
