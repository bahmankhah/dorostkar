...
class OpenAIProvider implements LLMProviderInterface
{
    ...
    public function testConnection()
    {
        if (empty($this->config['api_key'])) {
            return [
                'success' => false,
                'message' => __('کلید API پیکربندی نشده است', 'dorostkar'),
            ];
        }
        ...
        if (is_wp_error($response)) {
            return [
                'success' => false,
                'message' => __('خطا در اتصال: ', 'dorostkar') . $response->get_error_message(),
            ];
        }
        ...
        if ($statusCode === 401) {
            return [
                'success' => false,
                'message' => __('کلید API نامعتبر است', 'dorostkar'),
            ];
        }
        ...
        if ($statusCode === 403) {
            return [
                'success' => false,
                'message' => __('دسترسی رد شد. مجوزهای کلید API را بررسی کنید.', 'dorostkar'),
            ];
        }
        ...
        if ($statusCode !== 200) {
            return [
                'success' => false,
                'message' => sprintf(__('خطا در اتصال با کد وضعیت %d', 'dorostkar'), $statusCode),
            ];
        }

        return [
            'success' => true,
            'message' => __('اتصال به OpenAI با موفقیت برقرار شد', 'dorostkar'),
        ];
    }
