...
use Dorostkar\Modules\AIAgent\Contracts\AnalyzerInterface;
...
class ProductAnalyzer implements AnalyzerInterface
{
        ...
        foreach ($products as $product) {
            try {
                ...
                if ($analysis['success']) {
                    ...
                    $errorMsg = isset($analysis['error']) ? $analysis['error'] : __('خطای ناشناخته', 'dorostkar');
                    $results['errors'][] = [
    ...
    public function analyzeEntity($entityId, $dryRun = false)
    {
        ...
        if (!$product) {
            ...
            return ['success' => false, 'error' => __('محصول یافت نشد', 'dorostkar')];
        }
