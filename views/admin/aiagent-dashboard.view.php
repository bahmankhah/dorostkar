...
$baseUrl = admin_url('admin.php?page=dorostkar-ai-agent');
...
function aiagent_get_action_reasoning($action) {
    ...
    if (!empty($action['analysis_id'])) {
        $db = \Dorostkar\WPLite\Container::resolve(\Dorostkar\Modules\AIAgent\Services\DatabaseService::class);
...
<div class="wrap aiagent-dashboard" dir="rtl">
    ...
    <div class="notice notice-warning">
        <p><?php _e('ماژول دستیار هوشمند غیرفعال است. برای استفاده، آن را در تنظیمات فعال کنید.', 'dorostkar'); ?></p>
    </div>
    <?php endif; ?>
    ...
    <div id="analysis-progress-section" class="notice notice-info" style="display:none; padding: 15px;">
        <div class="progress-header" style="display: flex; justify-content: space-between; margin-bottom: 10px;">
            <span class="progress-status"><strong><?php _e('در حال تحلیل...', 'dorostkar'); ?></strong></span>
        ...
        <div class="progress-details" style="margin-top: 10px; display: flex; gap: 20px; color: #666;">
            <span id="progress-products"><?php _e('محصولات:', 'dorostkar'); ?> 0/0</span>
            <span id="progress-customers"><?php _e('مشتریان:', 'dorostkar'); ?> 0/0</span>
            <span id="progress-actions"><?php _e('اقدامات:', 'dorostkar'); ?> 0</span>
        </div>
    ...
    <div class="aiagent-status-bar" style="background: #fff; padding: 15px 20px; margin-bottom: 20px; border: 1px solid #ccd0d4; display: flex; justify-content: space-between; align-items: center;">
        <div class="status-info" style="display: flex; gap: 20px; align-items: center;">
            <span class="status-badge" style="padding: 5px 12px; border-radius: 4px; font-size: 12px; font-weight: 600; background: <?php echo $status['enabled'] ? '#d4edda' : '#f8d7da'; ?>; color: <?php echo $status['enabled'] ? '#155724' : '#721c24'; ?>;">
                <?php echo $status['enabled'] ? __('فعال', 'dorostkar') : __('غیرفعال', 'dorostkar'); ?>
            </span>
            <span style="color: #666;"><?php _e('سطح اشتراک:', 'dorostkar'); ?> <strong><?php echo esc_html($currentTierName); ?></strong></span>
            <span style="color: #666;"><?php _e('مدل:', 'dorostkar'); ?> <strong><?php echo esc_html($status['llm_model'] ?? '-'); ?></strong></span>
        </div>
    ...
    <div class="tab-content" style="background: #fff; border: 1px solid #ccd0d4; border-top: none; padding: 20px;">
        ...
        <div class="overview-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            ...
            <div class="overview-section">
                ...
                <table class="wp-list-table widefat fixed striped">
                    <thead>
                        <tr>
                            <th style="width: 30%;"><?php _e('نوع', 'dorostkar'); ?></th>
                            <th style="width: 20%;"><?php _e('وضعیت', 'dorostkar'); ?></th>
                            <th style="width: 15%;"><?php _e('اولویت', 'dorostkar'); ?></th>
                            <th style="width: 35%;"><?php _e('تاریخ', 'dorostkar'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php 
                        ...
                        <tr><td colspan="4" style="text-align: center; color: #666;"><?php _e('اقدامی در انتظار نیست', 'dorostkar'); ?></td></tr>
                ...
                <?php else: ?>
                <p style="color: #666; text-align: center;"><?php _e('هنوز اقدامی ثبت نشده است', 'dorostkar'); ?></p>
                <?php endif; ?>
            </div>
            ...
            <div class="overview-section">
                <h3 style="margin-top: 0; padding-bottom: 10px; border-bottom: 1px solid #eee;">
                    <?php _e('تحلیل‌های اخیر', 'dorostkar'); ?>
                ...
                <table class="wp-list-table widefat fixed striped">
                    <thead>
                        <tr>
                            <th style="width: 25%;"><?php _e('نوع', 'dorostkar'); ?></th>
                            <th style="width: 35%;"><?php _e('موجودیت', 'dorostkar'); ?></th>
                            <th style="width: 15%;"><?php _e('امتیاز', 'dorostkar'); ?></th>
                            <th style="width: 25%;"><?php _e('تاریخ', 'dorostkar'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($analysesData['items'] as $analysis): 
                            ...
                            if ($analysis['entity_type'] === 'product') {
                                ...
                                $entityName = $product ? $product->get_name() : __('محصول', 'dorostkar') . ' #' . $analysis['entity_id'];
                            } else {
                                $entityName = __('مشتری', 'dorostkar') . ' #' . $analysis['entity_id'];
                            }
                        ?>
                        <tr>
                ...
                <?php else: ?>
                <p style="color: #666; text-align: center;"><?php _e('هنوز تحلیلی انجام نشده است', 'dorostkar'); ?></p>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div style="margin-top: 30px;">
            <h3 style="margin-top: 0; padding-bottom: 10px; border-bottom: 1px solid #eee;">
                <?php _e('فعالیت ۳۰ روز گذشته', 'dorostkar'); ?>
            </h3>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
            <h3 style="margin: 0;"><?php _e('اقدامات در انتظار اجرا', 'dorostkar'); ?></h3>
            <?php if (!empty($actionsData['items'])): ?>
            <div style="display: flex; gap: 10px;">
                <button type="button" class="button button-primary" id="btn-approve-all">
                    <span class="dashicons dashicons-yes" style="vertical-align: middle;"></span>
                    <?php _e('تأیید همه', 'dorostkar'); ?>
                </button>
        ...
        <table class="wp-list-table widefat fixed striped">
            <thead>
                <tr>
                    <th style="width: 5%;"><?php _e('شناسه', 'dorostkar'); ?></th>
                    <th style="width: 18%;"><?php _e('نوع اقدام', 'dorostkar'); ?></th>
                    <th style="width: 8%;"><?php _e('اولویت', 'dorostkar'); ?></th>
                    <th style="width: 34%;"><?php _e('توضیحات هوش مصنوعی', 'dorostkar'); ?></th>
                    <th style="width: 15%;"><?php _e('تاریخ', 'dorostkar'); ?></th>
                    <th style="width: 20%;"><?php _e('عملیات', 'dorostkar'); ?></th>
                </tr>
            </thead>
            <tbody>
        ...
        <div class="tablenav bottom">
            <div class="tablenav-pages">
                <span class="displaying-num">
                    <?php printf(__('%s مورد', 'dorostkar'), number_format_i18n($actionsData['total'])); ?>
                </span>
                <?php
                echo paginate_links([
                    'base' => $paginationBase,
        ...
        <p style="text-align: center; color: #666; padding: 40px 0;">
            <?php _e('هیچ اقدامی در انتظار نیست.', 'dorostkar'); ?>
        </p>
        <?php endif; ?>
        ...
        <table class="wp-list-table widefat fixed striped">
            <thead>
                <tr>
                    <th style="width: 5%;"><?php _e('شناسه', 'dorostkar'); ?></th>
                    <th style="width: 15%;"><?php _e('نوع اقدام', 'dorostkar'); ?></th>
                    <th style="width: 10%;"><?php _e('وضعیت', 'dorostkar'); ?></th>
                    <th style="width: 8%;"><?php _e('اولویت', 'dorostkar'); ?></th>
                    <th style="width: 25%;"><?php _e('توضیحات هوش مصنوعی', 'dorostkar'); ?></th>
                    <th style="width: 15%;"><?php _e('تاریخ ایجاد', 'dorostkar'); ?></th>
                    <th style="width: 17%;"><?php _e('عملیات', 'dorostkar'); ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($actionsData['items'] as $action): 
        ...
        <?php else: ?>
        <p style="text-align: center; color: #666; padding: 40px 0;">
            <?php _e('هیچ اقدامی یافت نشد.', 'dorostkar'); ?>
        </p>
        <?php endif; ?>
        ...
        <div style="margin-bottom: 15px;">
            <form method="get" style="display: inline-flex; gap: 10px; align-items: center;">
                ...
                <select name="type" style="min-width: 150px;">
                    <option value=""><?php _e('همه انواع', 'dorostkar'); ?></option>
        ...
        <table class="wp-list-table widefat fixed striped">
            <thead>
                <tr>
                    <th style="width: 5%;"><?php _e('شناسه', 'dorostkar'); ?></th>
                    <th style="width: 10%;"><?php _e('نوع', 'dorostkar'); ?></th>
                    <th style="width: 20%;"><?php _e('موجودیت', 'dorostkar'); ?></th>
                    <th style="width: 8%;"><?php _e('امتیاز', 'dorostkar'); ?></th>
                    <th style="width: 35%;"><?php _e('تحلیل هوش مصنوعی', 'dorostkar'); ?></th>
                    <th style="width: 10%;"><?php _e('پیشنهادات', 'dorostkar'); ?></th>
                    <th style="width: 12%;"><?php _e('تاریخ', 'dorostkar'); ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($analysesData['items'] as $analysis): 
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <span class="suggestions-badge" 
                            ...
                            <?php echo count($suggestions); ?> <?php _e('پیشنهاد', 'dorostkar'); ?>
        ...
        <div class="tablenav bottom">
            <div class="tablenav-pages">
                <span class="displaying-num">
                    <?php printf(__('%s مورد', 'dorostkar'), number_format_i18n($analysesData['total'])); ?>
                </span>
                <?php
                echo paginate_links([
                    'base' => $paginationBase,
        ...
        <?php else: ?>
        <p style="text-align: center; color: #666; padding: 40px 0;">
            <?php _e('هنوز تحلیلی انجام نشده است. روی "شروع تحلیل" کلیک کنید.', 'dorostkar'); ?>
        </p>
        <?php endif; ?>

        <?php elseif ($currentTab === 'completed'): ?>
        <!-- Completed/History Tab -->
        <h3 style="margin-top: 0;"><?php _e('تاریخچه اقدامات انجام شده', 'dorostkar'); ?></h3>
        ...
        <table class="wp-list-table widefat fixed striped">
            <thead>
                <tr>
                    <th style="width: 5%;"><?php _e('شناسه', 'dorostkar'); ?></th>
                    <th style="width: 18%;"><?php _e('نوع اقدام', 'dorostkar'); ?></th>
                    <th style="width: 12%;"><?php _e('وضعیت', 'dorostkar'); ?></th>
                    <th style="width: 8%;"><?php _e('اولویت', 'dorostkar'); ?></th>
                    <th style="width: 25%;"><?php _e('توضیحات', 'dorostkar'); ?></th>
                    <th style="width: 16%;"><?php _e('تاریخ ایجاد', 'dorostkar'); ?></th>
                    <th style="width: 16%;"><?php _e('تاریخ اجرا', 'dorostkar'); ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($actionsData['items'] as $action): 
        ...
        <?php else: ?>
        <p style="text-align: center; color: #666; padding: 40px 0;">
            <?php _e('هنوز اقدامی انجام نشده است.', 'dorostkar'); ?>
        </p>
        <?php endif; ?>
...
<div id="suggestions-modal" class="aiagent-modal" style="display: none;">
    ...
    <div class="aiagent-modal-content" dir="rtl">
        <div class="aiagent-modal-header">
            <h3><?php _e('پیشنهادات هوش مصنوعی', 'dorostkar'); ?></h3>
        ...
        <div class="aiagent-modal-body">
            <table class="wp-list-table widefat fixed striped">
                <thead>
                    <tr>
                        <th style="width: 30%;"><?php _e('نوع اقدام', 'dorostkar'); ?></th>
                        <th style="width: 15%;"><?php _e('اولویت', 'dorostkar'); ?></th>
                        <th style="width: 55%;"><?php _e('توضیحات', 'dorostkar'); ?></th>
                    </tr>
                </thead>
...
jQuery(document).ready(function($) {
        ...
        if (ctx) {
            ...
            new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [
                        {
                            label: '<?php _e('تحلیل‌ها', 'dorostkar'); ?>',
                        ...
                        {
                            label: '<?php _e('اقدامات', 'dorostkar'); ?>',
    ...
    $('#run-analysis').on('click', function() {
        var $btn = $(this);
        $btn.prop('disabled', true).text('<?php _e('در حال شروع...', 'dorostkar'); ?>');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    alert(response.data.message || '<?php _e('خطا در شروع تحلیل', 'dorostkar'); ?>');
                    $btn.prop('disabled', false).text('<?php _e('شروع تحلیل', 'dorostkar'); ?>');
                }
            },
            error: function() {
                alert('<?php _e('خطا در ارتباط با سرور', 'dorostkar'); ?>');
                $btn.prop('disabled', false).text('<?php _e('شروع تحلیل', 'dorostkar'); ?>');
            }
        });
    });
    ...
    $('#cancel-analysis').on('click', function() {
        var $btn = $(this);
        $btn.prop('disabled', true).text('<?php _e('در حال لغو...', 'dorostkar'); ?>');
    ...
    $(document).on('click', '.btn-dismiss-action', function() {
        ...
        if (!confirm('<?php _e('آیا از حذف این اقدام اطمینان دارید؟', 'dorostkar'); ?>')) return;
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    alert(response.data.message || '<?php _e('خطا', 'dorostkar'); ?>');
                    $btn.removeClass('loading').text('<?php _e('حذف', 'dorostkar'); ?>');
                }
            },
            error: function() {
                alert('<?php _e('خطا در ارتباط با سرور', 'dorostkar'); ?>');
    ...
    $('#btn-approve-all').on('click', function() {
        if (!confirm('<?php _e('آیا از تأیید همه اقدامات در انتظار اطمینان دارید؟', 'dorostkar'); ?>')) return;
        
        var $btn = $(this);
        $btn.prop('disabled', true).text('<?php _e('در حال تأیید...', 'dorostkar'); ?>');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    alert(response.data.message || '<?php _e('خطا', 'dorostkar'); ?>');
                    $btn.prop('disabled', false).html('<span class="dashicons dashicons-yes" style="vertical-align: middle;"></span> <?php _e('تأیید همه', 'dorostkar'); ?>');
                }
            },
            error: function() {
                alert('<?php _e('خطا در ارتباط با سرور', 'dorostkar'); ?>');
    ...
    $('#btn-dismiss-all').on('click', function() {
        if (!confirm('<?php _e('آیا از حذف همه اقدامات در انتظار اطمینان دارید؟ این عمل قابل بازگشت نیست.', 'dorostkar'); ?>')) return;
        
        var $btn = $(this);
        $btn.prop('disabled', true).text('<?php _e('در حال حذف...', 'dorostkar'); ?>');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    alert(response.data.message || '<?php _e('خطا', 'dorostkar'); ?>');
                    $btn.prop('disabled', false).html('<span class="dashicons dashicons-no" style="vertical-align: middle;"></span> <?php _e('حذف همه', 'dorostkar'); ?>');
                }
            },
            error: function() {
                alert('<?php _e('خطا در ارتباط با سرور', 'dorostkar'); ?>');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    alert(response.data.message || '<?php _e('خطا', 'dorostkar'); ?>');
                    $btn.prop('disabled', false).html('<span class="dashicons dashicons-yes" style="vertical-align: middle;"></span> <?php _e('تأیید همه در انتظار', 'dorostkar'); ?>');
                }
            },
            error: function() {
                alert('<?php _e('خطا در ارتباط با سرور', 'dorostkar'); ?>');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    alert(response.data.message || '<?php _e('خطا', 'dorostkar'); ?>');
                    $btn.prop('disabled', false).html('<span class="dashicons dashicons-no" style="vertical-align: middle;"></span> <?php _e('حذف همه در انتظار', 'dorostkar'); ?>');
                }
            },
            error: function() {
                alert('<?php _e('خطا در ارتباط با سرور', 'dorostkar'); ?>');
    ...
    $(document).on('click', '.btn-execute-action', function() {
        ...
        if (!confirm('<?php _e('آیا از اجرای این اقدام اطمینان دارید؟', 'dorostkar'); ?>')) return;
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    var message = response.data.message || '<?php _e('اقدام با موفقیت اجرا شد', 'dorostkar'); ?>';
                    
                    // Update the row status or remove it
                    $row.find('.btn-execute-action, .btn-dismiss-action').remove();
                    $row.find('td:last').html('<span style="color: #46b450; font-weight: bold;"><?php _e('انجام شد', 'dorostkar'); ?></span>');
                    
                    // Update status badge if exists
                    var $statusBadge = $row.find('span[style*="background"]').first();
                    if ($statusBadge.length) {
                        $statusBadge.css('background', '#5cb85c').text('<?php _e('انجام شده', 'dorostkar'); ?>');
                    }
                    ...
                    var errorMsg = response.data.message || '<?php _e('خطا در اجرای اقدام', 'dorostkar'); ?>';
                    showNotification(errorMsg, 'error');
                    $btn.removeClass('loading').prop('disabled', false).text('<?php _e('اجرا', 'dorostkar'); ?>');
                }
            },
            error: function() {
                showNotification('<?php _e('خطا در ارتباط با سرور', 'dorostkar'); ?>', 'error');
                $btn.removeClass('loading').prop('disabled', false).text('<?php _e('اجرا', 'dorostkar'); ?>');
            }
        });
    });
    ...
    function updateProgressUI(data) {
        ...
        $('#progress-products').text('<?php _e('محصولات:', 'dorostkar'); ?> ' + (data.products_analyzed || 0) + '/' + (data.products_total || 0));
        $('#progress-customers').text('<?php _e('مشتریان:', 'dorostkar'); ?> ' + (data.customers_analyzed || 0) + '/' + (data.customers_total || 0));
        $('#progress-actions').text('<?php _e('اقدامات:', 'dorostkar'); ?> ' + (data.actions_created || 0));
        if (data.current_item) {
            $('#current-item').text('<?php _e('در حال تحلیل:', 'dorostkar'); ?> ' + data.current_item);
        }
    }
    
    function resetAnalysisUI() {
        $('#analysis-progress-section').hide();
        $('#cancel-analysis').hide().prop('disabled', false).text('<?php _e('لغو تحلیل', 'dorostkar'); ?>');
        $('#run-analysis').show().prop('disabled', false).text('<?php _e('شروع تحلیل', 'dorostkar'); ?>');
